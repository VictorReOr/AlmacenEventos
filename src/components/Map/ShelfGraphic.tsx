...
export const ShelfGraphic: React.FC<ShelfGraphicProps> = ({
    ...
    const C_STROKE = "#90A4AE";
    const TAB_THICKNESS = 8;

    // SVG def IDs
    const bodyGradId = `shelf-body-${u.id}`;
    const tabGradId  = `shelf-tab-${u.id}`;
    const clipId     = `shelf-clip-${u.id}`;

    const numModules = Math.round(u.width / SHELF_MODULE_WIDTH);

    // Separadores con sombra interior definida
    const dividers = [];
    for (let i = 1; i < numModules; i++) {
        if (rotationMode === 'vertical-ccw') {
            const xPos = -finalSvgW / 2 + (i * SHELF_MODULE_WIDTH * SCALE);
            dividers.push(<line key={`d-${i}`} x1={xPos} y1={-finalSvgH / 2 + TAB_THICKNESS} x2={xPos} y2={finalSvgH / 2} stroke="rgba(100,130,160,0.45)" strokeWidth={1.5} />);
        } else {
            const yPos = -finalSvgH / 2 + (i * SHELF_MODULE_WIDTH * SCALE);
            dividers.push(<line key={`d-${i}`} x1={-finalSvgW / 2 + TAB_THICKNESS} y1={yPos} x2={finalSvgW / 2} y2={yPos} stroke="rgba(100,130,160,0.45)" strokeWidth={1.5} />);
        }
    }
        ...
        <React.Fragment>
            ...
            >
                ...
                <defs>
                    ...
                    <clipPath id={clipId}>
                        <rect x={-finalSvgW / 2} y={-finalSvgH / 2} width={finalSvgW} height={finalSvgH} rx={3} />
                    </clipPath>
                </defs>

                {/* ── DROP SHADOW ── */}
                <rect x={-finalSvgW / 2} y={-finalSvgH / 2} width={finalSvgW} height={finalSvgH} fill="none" style={{ filter: 'drop-shadow(0px 3px 6px rgba(0,0,0,0.16))' }} />

                {/* ── SELECTION GLOW ── */}
                {isSelected && <rect x={-finalSvgW / 2 - 4} y={-finalSvgH / 2 - 4} width={finalSvgW + 8} height={finalSvgH + 8} fill="none" stroke="#2E7D32" strokeWidth={2.5} rx={4} style={{ filter: 'drop-shadow(0 0 4px #4CAF50)', animation: 'selPulse 1.4s ease-in-out infinite alternate' }} />}

                {/* ── BODY: slate-blue gradient ── */}
                <rect x={-finalSvgW / 2} y={-finalSvgH / 2} width={finalSvgW} height={finalSvgH} fill={`url(#${bodyGradId})`} stroke={C_STROKE} strokeWidth={1.5} rx={3} />

                {/* ── METALLIC GREEN TAB ── */}
                {rotationMode === 'vertical-ccw' ? (
                    <rect x={-finalSvgW / 2} y={-finalSvgH / 2} width={finalSvgW} height={TAB_THICKNESS} fill={`url(#${tabGradId})`} clipPath={`url(#${clipId})`} rx={2} />
                ) : (
                    <rect x={-finalSvgW / 2} y={-finalSvgH / 2} width={TAB_THICKNESS} height={finalSvgH} fill={`url(#${tabGradId})`} clipPath={`url(#${clipId})`} rx={2} />
                )}
                ...
                {Array.from({ length: numModules }).map((_, i) => {
                    ...
                    return (
                        <g key={`lbl-${moduleIndex}`} data-module={moduleIndex} data-shelf={u.id}>
                            {/* Hit area invisible para el módulo */}
                            <rect 
                                x={rotationMode === 'vertical-ccw' ? textX - (SHELF_MODULE_WIDTH * SCALE / 2) : -finalSvgW / 2}
                                y={rotationMode === 'vertical-ccw' ? -finalSvgH / 2 : textY - (SHELF_MODULE_WIDTH * SCALE / 2)}
                                width={rotationMode === 'vertical-ccw' ? SHELF_MODULE_WIDTH * SCALE : finalSvgW}
                                height={rotationMode === 'vertical-ccw' ? finalSvgH : SHELF_MODULE_WIDTH * SCALE}
                                fill="black"
                                fillOpacity={0.01}
                            ...
                            {/* Module number pill */}
                            <rect
                                x={(rotationMode === 'vertical-ccw' ? textX - 11 : textX - 15)}
                                y={(rotationMode === 'vertical-ccw' ? textY - 6   : textY - 6)}
                                width={rotationMode === 'vertical-ccw' ? 22 : 14}
                                height={12}
                                rx={3} ry={3}
                                fill="rgba(255,255,255,0.70)"
                                stroke="rgba(70,100,140,0.20)"
                                strokeWidth={0.5}
                                style={{ pointerEvents: 'none' }}
                            />
                            <text
                                x={textX - (rotationMode === 'vertical-ccw' ? 0 : 8)}
                                y={textY + 1}
                                fontSize={8.5}
                                fontWeight="800"
                                fill="#2E5080"
